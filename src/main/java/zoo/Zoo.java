...
public class Zoo {
    ...
    private boolean FileExtension(File animalsFile,String type){
        String fileName = animalsFile.getName();
        return fileName.endsWith(type);
    }

    public ObjectMapper mapperJson(File animalsFile) {
            ObjectMapper mapper = new ObjectMapper();
            return mapper;
    }

    public XmlMapper mapperXml(File animalsFile) {
    ...
    public void addAnimals(File animalsFile, String getTypeFile ) {
        AnimalsDataFile animalsData;
        try {
            if ( getTypeFile.equals("XML") == true  ) {
                animalsData = mapperXml(animalsFile).readValue(animalsFile, AnimalsDataFile.class);
            }
            else{
                animalsData = mapperJson(animalsFile).readValue(animalsFile, AnimalsDataFile.class);
            }
    ...
    public void addAnimalsFile(String Path, String getTypeFile ) {
        File animalsFile = new File(Path);
        addAnimals(animalsFile,getTypeFile );
    }
